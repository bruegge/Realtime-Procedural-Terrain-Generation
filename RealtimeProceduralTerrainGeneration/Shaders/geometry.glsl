...
{    
    ...
    gl_Position = projectionMatrix * viewMatrix * modelMatrix * vec4(gl_in[0].gl_Position.xyz,1); 
    ...
    gl_Position = projectionMatrix * viewMatrix * modelMatrix * vec4(gl_in[1].gl_Position.xyz,1);
	gs_out.vertexTextureCoordinates = gs_in[1].vertexTextureCoordinates;
    gs_out.vertexNormal = faceNormal;
    gs_out.vertexPosition = gl_in[1].gl_Position.xyz;
	EmitVertex();
   
    gl_Position = projectionMatrix * viewMatrix * modelMatrix * vec4(gl_in[2].gl_Position.xyz,1);
